...
The USB-PD library is developed following the Universal Serial Bus Power Delivery Specification Revision 3.2, V1.1 (October 2024) and Universal Serial Bus type-C Cable
...
### Official support of Universal Serial Bus Power Delivery Specification Revision 3.2
### Maintenance Release
...
**New Features Supported**
  ...
  --------
  AVS in SPR mode
  Dynamic Power Sources (DPS)
  Deprecate AMS interruptibility
  PR_Swap in EPR mode is disallowed
  Deprecate GotoMin and GiveBack Features
...
**Fixed bugs list**
  ...
  Headline
  --------
    PE_SRC_WAIT_NEW_CAPABILITIES state freeze fixed
    tPSSourceOff value updated
    Adapt the goodCRC message header to select the proper specification revision

## Known limitations
  Headline
  --------
  ECN : Wait in EPR_Request not implemented

## Development Toolchains and Compilers

- IAR Embedded Workbench for ARM (EWARM) toolchain V9.30.1
- Keil Microcontroller Development Kit (MDK-ARM) toolchain V5.38
...
**Fixed bugs list**
  ...
  Headline
  --------
    Split tSenderResponse timing for PD2 and PD3 messages
    Implement USBPD_TRACE_DeInit
    Split os queues for each port
    Replace wrong call to CMSIS RTOS API version for TCPM code section
	Improve error recovery reponse time after failure to send PS_RDY
	Implement verification of CopyPDO from an EPR Request with SRC PDOs sent by SRC in last SRC Capabilities
...
**Fixed bugs list**
  ...
  --------
    ...
    Add CM55 support
    Add defines of Power Delivery revision 
	...
	Fix data role swap from device to host when port power role is source
